class isolde {
  ...
  orderelements() {
    let { parent, activeElements, columns, blocWidth, responsive, margin } =
      this;
    ...
    let arrayRectHeight = activeElements.reduce((acc, el, id) => {
      ...
      let rectHeight =
        id - columns >= 0
          ? columnsHeight[id % columns] + margin * Math.floor(id / columns)
          : 0;
    ...
    let parentHeight =
      Math.max(...columnsMaxHeight) +
      margin * (Math.floor(activeElements.length / columns) - 1);
  ...
  _setBlocWidth(callback) {
    ...
    let blocWidth = (this.blocWidth =
      (parent.clientWidth - margin * (columns - 1)) / columns);
